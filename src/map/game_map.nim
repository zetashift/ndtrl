import tile, rect, ../entity, random
...
method generateMap*(map: Map, maxRooms, roomMinSize, roomMaxSize: int, player: Entity) {.base.} =
  randomize()
  var
    rooms = newSeq[Rect]()
    numRooms = 0
  ...
  for r in 0 ..< maxRooms:
    ...
    block checkRooms:
      for otherRoom in rooms:
        if newRoom.intersect(otherRoom): break checkRooms
        else:
          map.createRoom(newRoom)
          let (newX, newY) = newRoom.center()
          # Check if this is the first room created if so spawn the player
          if numRooms == 0:
            player.x = newX
            player.y = newY
          else:
